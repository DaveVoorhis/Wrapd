...
public abstract class UpdatableTuple extends Tuple implements Cloneable {
    private final static Msg ErrTupleNotInsertableBecauseNullDatabase = new Msg("Tuple is not insertable, because this Tuple was not constructed with a Database argument.", UpdatableTuple.class);
    private final static Msg ErrTupleNotUpdatableBecauseNullDatabase = new Msg("Tuple is not updatable, because this Tuple was not constructed with a Database argument.", UpdatableTuple.class);
    private final static Msg ErrTupleNotUpdatable = new Msg("Tuple is not updatable. Tuples become updatable by invoking backup() after population and before mutation, usually by being obtained via Database::queryForUpdate or Database::queryAllForUpdate.", UpdatableTuple.class);
    ...
    public List<FieldGetFailure> insert(Connection connection, String tableName) throws SQLException {
        if (database == null)
            throw new InvalidValueException(Str.ing(ErrTupleNotInsertableBecauseNullDatabase));
    ...
    public Pair<List<FieldGetFailure>, List<FieldGetFailure>> update(Connection connection, String tableName) throws SQLException {
        if (database == null)
            throw new InvalidValueException(Str.ing(ErrTupleNotUpdatableBecauseNullDatabase));
        var backup = getBackup();
        if (backup == null)
            throw new InvalidValueException(Str.ing(ErrTupleNotUpdatable));
